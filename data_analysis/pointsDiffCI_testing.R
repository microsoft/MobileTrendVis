...
mu1 <- runif(n_exp, min = -10, max = 10)
if (same_mu) {mu2 <- mu1} else {mu2 <- runif(n_exp, min = -10, max = 10)}
...
for (e in 1:n_exp) {
  ...
  for (r in 1: n_replications) {
    sample1.df <- data.frame(rnorm(n = n1[e], mean = mu1[e], sd= sigma1[e]))
    sample1.df$condition <- 'animation'
    colnames(sample1.df) <- c('points','condition')
    sample2.df <- data.frame(rnorm(n= n2[e], mean = mu2[e], sd = sigma2[e]))
    sample2.df$condition <- 'multiples'
    colnames(sample2.df) <- c('points','condition')
    sample.df <- rbind(sample1.df,sample2.df)
    sample.df$condition <- sample.df$condition %>% as.factor()
    # message("min: ", min(sample.df$points))
    ci <- bootdif(sample.df)
    if (mu1[e] > mu2[e]) {true_diff <- mu1[e] - mu2[e]} else {true_diff <- mu2[e] - mu1[e]}
